...
class db{
	...
	public function get_reindex_string($table,$index,$cols){
		if(!is_array($cols))   //检测变量是否是数组
			$this->queries='SELECT '.$cols.','.$index.' FROM '.$this->prefix.$table;
		else{
			$colss=implode(',',$cols);   //用“，”将一维数组$cols连接为一个字符串
		...
		foreach ($rows as $row) {
			...
			if(empty($rerows[$row[$index]])){
				...
				if(!is_array($cols)){
					...
					$rerows[$row[$index]].=', ';  
					$rerows[$row[$index]].=substr($str,1);	//返回字符串的子串
				}
			}
		}
		return $rerows;
	...
	******************/
	...
	//向关系表中添加用户id和组id
	public function add_id($user_name,$user_gname) {
		$user_id = $this->get_user_id($user_name);
		$user_gid = $this->get_user_gid($user_gname);
		//$usergroup['user_gid']	
		return $this->insert_table('ugu',array('user_id'=>"'$user_id'",'user_gid'=>"'$user_gid'"));
	}

	//更改用户表中的组id
	public function update_user_gid($user_name,$user_gname) {
		$user_id = $this->get_user_id($user_name);
		$user_gid = $this->get_user_gid($user_gname);
		$colvs = array('user_gid'=>$user_gid);
		return $this->update_table('users',$colvs,array('user_id'=>$user_id));
	}

	//清空用户表中的组id
	public function clear_user_gid($user_name) {
		$user_id = $this->get_user_id($user_name);
		$colvs = array('user_gid'=>'NULL');
		return $this->update_table('users',$colvs,array('user_id'=>$user_id));
	}
}
?>